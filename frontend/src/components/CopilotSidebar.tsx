...
function suggestions(scope: { obligationId?: string; documentId?: string }): string[] {
  if (scope.obligationId)
    return ["Why was this extracted?", "Explain this obligation in plain English", "What evidence is required and who owns it?"];
  if (scope.documentId)
    return ["Summarize this circular", "Which departments are affected?", "Show the cybersecurity obligations"];
  return ["Show all technology obligations", "Find obligations that need review", "What has the pipeline done recently?"];
}
    ...
    <aside className="fixed right-3 top-3 bottom-3 z-30 flex w-96 flex-col rounded-2xl border bg-card shadow-xl overflow-hidden animate-in">
      <div className="flex items-center gap-2 border-b px-4 h-14 shrink-0">
        ...
        <div className="leading-tight">
          ...
          <div className="text-[10px] text-muted-foreground">grounded in your compliance data</div>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-sm text-muted-foreground">
            Ask about obligations, circulars, owners, deadlines or risk. Answers are generated by the
            local model over your real compliance records.
          </div>
        ) : (
          messages.map((m, i) => (
