...
The relational store (`db/models.py`) is the compliance system of record (proposal §5.2.3,
§10.3). It is Postgres-first but type-portable (JSON, String, Date), so the same models run
on SQLite for no-Docker development. SQLAlchemy 2.x ORM; `db/session.py` builds the engine and
sessions; `db/crud.py` centralises writes and audit logging.
...
| `funnel` | JSON | per-run funnel stats (see [processing-funnel.md](processing-funnel.md#funnel-observability)) |
...
Diff key: `(family_key, section_label)` → `content_hash`. See
[processing-funnel.md](processing-funnel.md#layer-4--diff-engine-preprocessingfingerprintpy).

### `audit_log`
Append-only trail (§10.3) — inserts only; never updated or deleted in application code.
...
Every mutating helper in `db/crud.py` writes an `audit_log` row (`record_audit`). The pipeline
never writes to the ORM directly — it goes through CRUD — so audit coverage is centralised and
complete. A typical processed circular yields audit rows for ingestion, each obligation
extraction, each approval/edit, each rule and task, and each report generation.
